...
def main():
    ...
    commoninterests = { (a, b): interests[a] & interests[b] for a in interests
    for b in interests if a < b } 
    ...
    lowJaccard = [p for p in commoninterests
    if len(commoninterests[p]) / len(interests[p[0]] | interests[p[1]]) < 0.25]
    print(lowJaccard)
